...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])) {
    ...
    if (isset($_FILES['gambar_baru']) && $_FILES['gambar_baru']['error'] == UPLOAD_ERR_OK && $_FILES['gambar_baru']['size'] > 0) {
        $target_dir = "assets/img/penyakit/"; // Direktori tujuan upload
    ...
    if ($uploadOk == 1) {
        try {
            ...
            // Eksekusi query
            if ($stmt->execute([$nama_penyakit, $gambar_untuk_db, $informasi, $gejala_detail, $solusi, $id])) {
                ...
                echo "Gagal mengupdate data penyakit di database.";
            }
